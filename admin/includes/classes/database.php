...
-- Database: `weblog`
--
CREATE DATABASE IF NOT EXISTS `weblog` DEFAULT CHARACTER SET utf8 COLLATE utf8_persian_ci;
USE `weblog`;
...
class Database {
    ...
    public function open_db_connection(){
        ...
        if($this->connection->connect_errno){
            die('database connection faild badly ' . mysqli_error());
        }
    }
    ...
    public function query($sql) {
        ...
        if(!$result){
            die('Query faild'.$sql.'<br>'.  mysqli_error($this->connection));
        }
        return $result;
    }
